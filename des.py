...
from utils import initial_permutation, final_permutation, expand, permute, xor_bits, feistel_function, substitute

# Fungsi-fungsi tambahan seperti Feistel, S-box, dll
S_BOX = [
...
def feistel_function(R, key):
    # Implementasikan fungsi Feistel (Expansion + XOR + S-Box + Permutasi P)
    # Ekspansi
    expanded_R = expand(R)  # Ekspansi R dari 32 bit ke 48 bit
    # XOR
    xor_output = xor_bits(expanded_R, key)
    # Substitusi menggunakan S-Box
    substituted_output = substitute(xor_output)
    # Permutasi P
    return permute(substituted_output, P)
